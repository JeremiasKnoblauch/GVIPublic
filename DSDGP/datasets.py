...
# Salimbeni & Deisenroth '17. What I have changed here is that the data is
# stored LOCALLY (so no downloads are initiated.) I have also added some other
# data sets in.
...
class Dataset(object):
    ...
    def read_data(self):
        if self.name in ['naval', 'wine', 'yacht']:
            delim = ' '
            data = np.loadtxt(self.data_path + "/" + self.name + '.txt')
        else:
            delim = ','
            data = pandas.read_csv(self.csv_file_path(self.name),
                               header=None, delimiter=delim).values
...
class Kin8mn(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'kin8mn', 8192, 8
        self.type = 'regression'
...
class WineRed(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'wine', 1599, 11
        self.type = 'regression'
...
class Yacht(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'yacht', 308, 6
        self.type = 'regression'
...
class Datasets(object):
    def __init__(self, data_path='/Users/jeremiasknoblauch/Documents/OxWaSP/GVI/code/DSDGP/demos/data/'): #'/data/'):
        print(data_path)
